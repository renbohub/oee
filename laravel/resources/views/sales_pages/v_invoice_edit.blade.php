                    ...
                    <div class="card-header">
                        <div class="row">
                            <div class="col-12 mb-2">
                                <h2>Invoice/ <u>{{$header->invoice_tittle}}</u> / <u>{{$header->quotation_name}}</u> / <u>@if ($header->invoice_number == '') Draft @else {{$header->invoice_number}} @endif</u></h2> 
                            </div>
                            <div class="col-6">
                                <div class="text-left">
                                    @if ($header->invoice_status == 1)
                                        <button class="btn btn-success mb-3" onclick="setApprove(2)">Request Approval</button>
                                    @elseif($header->invoice_status == 2)
                                        @if($role==1)
                                        <button class="btn btn-primary mb-3" onclick="setApprove(3)">Approve Quotation</button>
                                        <button class="btn btn-danger mb-3" onclick="setApprove(1)">Reject Quotation</button>
                                        @endif
                                        <button class="btn btn-success mb-3" onclick="setApprove(1)">Edit Invoice</button>
                                    @else
                                         <button class="btn btn-success mb-3" onclick="setApprove(1)">Edit Invoice</button>
                                    @endif       
                                </div>
                            ...
                            <div class="col-6">
                                <div class="text-right">
                                    ...
                                    @if($header->invoice_status > 2)
                                    <a target="_blank" href="{{route('sales-invoice-export',$header->invoice_token)}}" class=" btn btn-warning mb-3">Print</a>&nbsp
                                    @endif
                            ...
                            <div class="col-12 col-sm-12 col-md-12 col-lg-6 border mb-1">
                                <div class="row">
                                    <div class="col-12 col-sm-12 col-md-12 col-lg-4 border text-center @if ($header->invoice_status == 1) bg-warning @endif">
                                        Draft
                                    </div>
                                    <div class="col-12 col-sm-12 col-md-12 col-lg-4 border text-center @if ($header->invoice_status == 2) bg-warning @endif">
                                        Wait Approval
                                    </div>
                                    <div class="col-12 col-sm-12 col-md-12 col-lg-4 border text-center @if ($header->invoice_status >= 3) bg-success @endif">
                                        Invoice Approved
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="card-body pr-2">
                        <div class="row">
                            ...
                            <div class="col-12 col-md-12 col-lg-6"> 
                                <div class="form-group">
                                  ...
                                  <input type="text"
                                    class="form-control" disabled name="" id="so_cust_po" onchange="updateCustPO()" aria-describedby="helpId" placeholder="" value="{{$header->so_po_customer}}" disbled>
                            ...
                            <div class="col-12 col-md-12 col-lg-6">
                                <div class="form-group row mb-1">
                                    <label for="staticEmail" class="col-3 col-form-label">Invoice Title</label>
                                    <div class="col-9">
                                      <input type="text" @if($header->invoice_status > 1) disabled @endif onchange="updateHeaders()" class="form-control" id="h_invoice_tittle" value="{{$header->invoice_tittle}}">
                                    </div>
                                </div>
                                <div class="form-group row mb-1">
                                    <label for="staticEmail" class="col-3 col-form-label">Invoice Rate</label>
                                    <div class="col-9">
                                      <input type="number" @if($header->invoice_status > 1) disabled @endif onchange="updateHeaders()"  max="{{ 100 - $header->invoice_created }}" class="form-control" id="h_invoice_rate" value="{{$header->invoice_rate}}">
                                    </div>
                                </div>
                                <div class="form-group row mb-1">
                                    <label for="staticEmail" class="col-3 col-form-label">Invoice Date</label>
                                    <div class="col-9">
                                      <input type="date" @if($header->invoice_status > 1) disabled @endif onchange="updateHeaders()" class="form-control" id="h_invoice_date" value="{{ \Carbon\Carbon::parse($header->invoice_date)->format('Y-m-d') }}">
                                    </div>
                                </div>
                                <div class="form-group row mb-1">
                                    <label for="staticEmail" class="col-3 col-form-label">Invoice Due Date</label>
                                    <div class="col-9">
                                      <input type="date" @if($header->invoice_status > 1) disabled @endif onchange="updateHeaders()" class="form-control" id="h_invoice_exp" value="{{ \Carbon\Carbon::parse($header->invoice_exp)->format('Y-m-d') }}">
                                    </div>
                                </div>
                                <div class="form-group row mb-1">
                                    <label for="staticEmail" class="col-3 col-form-label">Payment Method</label>
                                    <div class="col-9">
                                        <select @if($header->invoice_status > 1) disabled @endif class="form-control border-top-1 select2_demo_1" onchange="updateHeaders()" id="h_account_id" style="width: 100%">
                                            @foreach ($accounts as $item)
                                                <option value="{{$item->account_id}}" @if($item->account_id == $header->account_id) selected @endif>{{$item->account_name}}-({{$item->account_bank}})</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <tbody id="sortable">
                                        @foreach ($sub as $sub)
                                            ...
                                            <tr id='row_{{$sub->quotation_order}}'>
                                                ...
                                                <td class='py-0 px-0'><input @if($header->quotation_status > 1) readonly @endif type='text' placeholder='0' value='{{ number_format($sub->quotation_price * ($header->invoice_rate/100), 0, '.', ',') }}' id='price{{$sub->quotation_order}}' name='quotation_price[]' onchange='priceNumber({{$sub->quotation_order}})' class='text-right form-control border-none bg-none rounded-0 price-number' required> </td>
                                                <td class='py-0 px-0'><input  type='text' placeholder='0' value='{{ number_format($sub->quotation_total * ($header->invoice_rate /100), 0, '.', ',') }}'  id='total{{$sub->quotation_order}}'  name='quotation_total[]' class='text-right form-control border-none bg-none rounded-0' readonly> </td>
                                                <td class='py-0 px-0'><button type='button' class='btn bg-none' onclick='removeRow({{$sub->quotation_order}})'><i class='fa fa-trash'></i></button></td>
                                            </tr>
                                            @else
                                            <tr id='row_{{$sub->quotation_order}}'>
                        ...
                        <div class="row">
                            ...
                            <div class="col-12 col-sm-12 col-md-6 col-lg-8 d-none d-md-block d-sm-none">
                                <div class="form-group">
                                  <textarea type="text" onchange="updateHeaders()" class="form-control" id="h_invoice_note" rows="3">{{$header->invoice_note}}</textarea>
                                </div>
                            </div>
...
@section('script')
    ...
    <script>
        ...
        function updateHeaders(){
            var invoice_tittle = document.getElementById('h_invoice_tittle').value;
            var invoice_exp = document.getElementById('h_invoice_exp').value;
            var invoice_date = document.getElementById('h_invoice_date').value;
            var invoice_rate = document.getElementById('h_invoice_rate').value;
            var account_id = document.getElementById('h_account_id').value
          
            $.ajax({
                url: '{{route("sales-invoice-edit-header")}}',
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                    id: '{{$header->invoice_token}}',
                    invoice_tittle: invoice_tittle,
                    invoice_date:invoice_date,
                    invoice_exp:invoice_date,
        ...
        function setApprove(status){
            ...
            $.ajax({
                url: '{{route("sales-invoice-update-status")}}',
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                    id: '{{$header->invoice_token}}',
