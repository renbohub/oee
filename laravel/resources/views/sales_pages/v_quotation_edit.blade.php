                    ...
                    <div class="card-header">
                        <div class="row">
                            <div class="col-12 mb-2">
                                <h2>Quotation / <u>{{$header->quotation_name}}</u> / <u>@if ($header->quotation_number == '') Drafted @else {{$header->quotation_number}} @endif</u></h2> 
                            </div>
                            <div class="col-6">
                                <div class="text-left">
                                    @if ($header->quotation_status == 1)
                                        <button class="btn btn-success mb-3" onclick="setApprove(2)">Request Approval</button>
                                    @elseif($header->quotation_status == 2)
                                        @if($role==1)
                                        <button class="btn btn-primary mb-3" onclick="setApprove(3)">Approve Quoatation</button>
                                        <button class="btn btn-danger mb-3" onclick="setApprove(1)">Reject Quotation</button>
                                        @endif
                                        <button class="btn btn-success mb-3" onclick="setApprove(1)">Back to Draft</button>
                                    @elseif($header->quotation_status == 3)
                                        <button class="btn btn-primary mb-3" onclick="setApprove(4)">Mark Quotation as Send</button>
                                        <button class="btn btn-success mb-3" onclick="setApprove(1)">Back to Draft</button>
                                    @elseif($header->quotation_status == 4)
                                        <!-- Button trigger modal -->
                                        <button type="button" class="btn btn-primary btn-lg mb-3" data-toggle="modal" data-target="#modelId"> Set To SO
                                        </button>
                                        
                                        <button class="btn btn-warning btn-lg mb-3" onclick="setApprove(1)">Back to Draft</button>
                                    @elseif($header->quotation_status == 5)
                                    @endif        
                                </div>
                            </div>
                            <div class="col-6">
                                <div class="text-right">
                                    ...
                                    <a target="_blank" href="{{route('sales-quotation-export',$header->quotation_token)}}" class=" btn btn-warning mb-3">Print</a>&nbsp
                                    @endif
                                    <a  href="{{route('sales-quotation')}}" class=" btn btn-primary mb-3">Back</a>&nbsp
                                </div>
                            </div> 
                            ...
                            <div class="col-12 col-sm-12 col-md-12 col-lg-6 border mb-1">
                                <div class="row">
                                    <div class="col-12 col-sm-12 col-md-12 col-lg-3 border text-center @if ($header->quotation_status == 1) bg-warning @endif">
                                        Drafted
                                    </div>
                                    <div class="col-12 col-sm-12 col-md-12 col-lg-3 border text-center @if ($header->quotation_status == 2) bg-info @endif">
                                        Wait Approval
                                    </div>
                                    <div class="col-12 col-sm-12 col-md-12 col-lg-3 border text-center @if ($header->quotation_status == 3) bg-info @endif">
                                        Approved
                                    </div>
                                    <div class="col-12 col-sm-12 col-md-12 col-lg-3 border text-center @if ($header->quotation_status >= 4) bg-info @endif">
                                        Quoatation Send
                                    </div>
    ...
    <div class="modal fade" id="modelId" tabindex="-1" role="dialog" aria-labelledby="modelTitleId" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{route('sales-order-post')}}" method="post">
                    <div class="modal-body">
                        @csrf
                        <div class="form-group">
                        <label for="">Input Customer PO</label>
                        <input type="text" name="so_po_customer" id="" class="form-control" placeholder="" aria-describedby="helpId" required>
                        <small id="helpId" class="text-muted">Input PO Customer</small>
                        </div>
                        <div class="form-group">
                        <label for="">SO Date</label>
                        <input type="date"
                            class="form-control" name="so_date" id="" aria-describedby="helpId" placeholder="" required>
...
@section('script')
    ...
    <script>
        ...
        function setApprove(status){
            ...
            $.ajax({
                url: '{{route("sales-quotation-update-status")}}',
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                    id: '{{$header->quotation_token}}',
                    status: status
                },
                success: function(data) {
                    location.reload();
