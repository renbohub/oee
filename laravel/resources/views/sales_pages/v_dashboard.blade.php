...
@section('script')
    <script>
        ...
        var q_price = 0
        for (let i = 0; i < q.quotation.length; i++) {
            q_price += parseFloat(q.quotation[i].y);

        }
        q_p = String(q_price.toFixed(0)).replace(/\B(?=(\d{3})+(?!\d))/g, ',')

        var i_price = 0
        for (let i = 0; i < q.invoice.length; i++) {
            i_price += parseFloat(q.invoice[i].y);

        }
        i_p = String(i_price.toFixed(0)).replace(/\B(?=(\d{3})+(?!\d))/g, ',')

        var s_price = 0
        for (let i = 0; i < q.sales_order.length; i++) {
            s_price += parseFloat(q.sales_order[i].y);
